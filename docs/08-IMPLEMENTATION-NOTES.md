...
- **変更判定は純粋関数 `ChangeDetector`（`preDecision`/`postHash`）に集約**し、`performFullScan` / `processEventToQueue` の両経路で共用（docs/04a の SHA ゲート仕様の実装）。size 一致 + mtime 不一致のときだけ SHA を再計算し、一致なら **CAS（`LocalDatabase.refreshMtimeIfShaUnchanged`）で mtime のみ修復**してアップロードしない（`lastSyncedAt` 保持・sha 不一致なら no-op = 並行 pull の更新を巻き戻さない）。size 不一致は hash せず直接 enqueue（sha が一致し得ないため仕様と同義）。`processEventToQueue` は @MainActor なので **hash は `Task.detached` 経由**。
...
- **バージョン単一ソース化**: `Tide/Info.plist` の `CFBundleShortVersionString` / `CFBundleVersion` を `$(MARKETING_VERSION)` / `$(CURRENT_PROJECT_VERSION)` 参照にし、**バージョンは `project.yml` の build settings を唯一のソース**にした（plist 側に数値を二重定義しない・Xcode がビルド時に展開）。あわせて `NSHumanReadableCopyright` を設定、`LSApplicationCategoryType=utilities` を追加（アーカイブ時のカテゴリ未設定警告を解消）。About 表示・診断テキストはどちらも `Bundle.main` から動的取得するのでこの単一ソースに追従する。**適用範囲の拡張（2026-09-19・Issue #112）**: `MARKETING_VERSION` / `CURRENT_PROJECT_VERSION` は Tide ターゲット限定だったため `TideFileProvider.appex` が xcodegen 既定の 1.0 に落ち、`CFBundleShortVersionString of an app extension ('1.0') must match that of its containing parent app` の build warning が 0.1.1 以降ずっと潜在していた（配布・公証では検証エラーに格上げされ得る）。両キーと版運用ルールのコメントを**プロジェクトレベル `settings.base` へ引き上げて全ターゲット共有**にし、拡張の `info.properties` にも app と同じ `$(…)` 参照を追加。`TideCore.framework`（`GENERATE_INFOPLIST_FILE`）も同じ値を拾う。
