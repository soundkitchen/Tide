...
- **残存レース（いずれも data loss でない・versioning backstop）**: (1) 本体 PUT は判定前に走るので `.conflict` 時にマニフェスト未参照の orphan S3 version が 1 つ残る（すべての読みは manifest の versionId 経由なので orphan は配信されない）。(2) 2 台同時 `.conflict` は互いのコピーができ得る（稀・自己収束）。
