...
### launchd 常駐化（Issue #84・常駐運用は 2026-09-19 で終了）

> **常駐 soak は 2026-09-19 で終了**（#40 合格判定後 1,980 周回 persistent DRIFT ゼロ・根拠と
> 経緯 = `docs/09` #40 節）。以後は `make soak-check-fp` の**手動スポットチェックのみ**（節目で
> 1 回）。本節は再 soak が要るとき（2 台 soak・大規模変更後）に再導入するための手順として温存。

`--fp-only --watch` の駆動は **LaunchAgent 常駐が標準**だった（2026-07-25〜2026-09-19）。マシン再起動後の
...
- **対象外（残余ギャップ・PR #95 レビュー指摘）**: 本通知が検出するのは **watch ループ到達後の
  周回失敗**のみ。起動時の設定解決失敗（TCC 拒否・group defaults 不達等 → `exit 2`）で
  launchd が 60 秒スロットルの再スポーンを繰り返すケースは、ループ到達前にプロセスが死に
  notifier の状態も毎回消えるため**通知されない**。このクラスの空白は従来どおり
  `make soak-agent-status` と `agent.err.log` で確認する（上記「launchd 固有の TCC」参照）。
