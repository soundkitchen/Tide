...
public enum HashCalculator {
    ...
    public static func sha256(of url: URL, chunkSize: Int = 65_536) throws -> String {
    ...
    public static func sha256NoFollow(of url: URL, chunkSize: Int = 65_536) throws -> String {
    ...
    public static func readAllAndHash(_ reader: NoFollowFileReader, chunkSize: Int = 1 << 20) throws -> (data: Data, sha256: String) {
        var hasher = SHA256()
        var data = Data()
        while let chunk = try reader.readChunk(chunkSize) {
            hasher.update(data: chunk)
