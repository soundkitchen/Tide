...
public struct SyncIgnoreMatcher: Sendable {
    /// `.syncignore` のバイト数上限。これを超える分は読み込み側で打ち切る。
    public static let maxBytes = 256 * 1024
    /// パターン数上限。超過分は捨てる。
    public static let maxPatterns = 10_000
    ...
    public static let empty = SyncIgnoreMatcher(patterns: [], sourceLines: [])
    ...
    public static func parse(_ text: String) -> SyncIgnoreMatcher {
    ...
    public func evaluate(_ relativePath: String) -> Verdict {
...
public struct LayeredSyncIgnore: Sendable {
    ...
    public init(matchers: [String: SyncIgnoreMatcher]) {
