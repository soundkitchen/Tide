...
public enum SyncIssueClassifier {
    public static func classify(error: Error, path: String? = nil, date: Date = Date()) -> SyncIssue {
        SyncIssue(
            id: UUID(),
            date: date,
            path: path,
    ...
    public static func category(for error: Error) -> SyncIssue.Category {
