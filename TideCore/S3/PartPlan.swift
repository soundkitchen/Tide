...
public enum PartPlan {
    /// S3 マルチパートの最小パートサイズ（最終パートを除く）。
    public static let minPartSize: Int = 5 * 1024 * 1024            // 5 MiB
    /// パートサイズの上限。常駐メモリ（≈ partSize ×（inflight+1））を抑えるための cap（L11）。
    /// これを超えるのは、10,000 パート上限に収まらない超巨大ファイル（おおむね 640GiB 超）だけ。
    public static let maxPartSize: Int = 64 * 1024 * 1024           // 64 MiB
    /// S3 マルチパートの最大パート数。
    public static let maxPartCount: Int = 10_000
    /// これを超えたらマルチパート、以下ならシングル PutObject。
    public static let multipartThreshold: Int64 = 16 * 1024 * 1024  // 16 MiB
    /// アダプティブ計算で目標とするパート数（10,000 の上限に対して余裕を持たせる）。
    public static let targetMaxParts: Int64 = 9_000
    ...
    public struct Plan: Equatable {
        ...
        public init(partSize: Int, partCount: Int) {
    ...
    public static func plan(forFileSize fileSize: Int64) -> Plan {
    ...
    public static func isWithinUploadLimit(size: Int64, limitBytes: Int64) -> Bool {
