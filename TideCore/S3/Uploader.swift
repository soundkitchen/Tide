...
public struct Uploader: Sendable {
    ...
    private func processUpload(_ item: UploadQueueRecord) async throws {
        ...
        do {
            ...
            return
        } catch FileOpenError.notFound, FileOpenError.isDirectory, FileOpenError.io(errno: ENOTDIR) {
            ...
            // ENOTDIR を追加し catch を統合）。
            AppLogger.sync.info("Path no longer names a regular file; converting upload to delete: \(path, privacy: .private)")
