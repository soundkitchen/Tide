...
public protocol MultipartUploadClient: Sendable {
...
public struct MultipartUploader {
    public let s3: any MultipartUploadClient
    /// パート単位リトライの方針（既定は本番値。テストは短い遅延を注入して高速化する）。
    public var retryPolicy: RetryPolicy = .default

    public init(s3: any MultipartUploadClient, retryPolicy: RetryPolicy = .default) {
        self.s3 = s3
        self.retryPolicy = retryPolicy
    }

    /// 同時に in-flight にするパート数の上限（メモリ ≈ partSize × (maxInflight + 1)）。
    public static let maxInflightParts = 3
    ...
    public struct RetryPolicy: Sendable {
        ...
        public init(maxAttempts: Int, baseDelaySeconds: Double, maxDelaySeconds: Double) {
            self.maxAttempts = maxAttempts
            self.baseDelaySeconds = baseDelaySeconds
            self.maxDelaySeconds = maxDelaySeconds
        }

        public static let `default` = RetryPolicy(maxAttempts: 3, baseDelaySeconds: 2, maxDelaySeconds: 30)
    }

    public struct Result: Sendable {
        public let put: TideS3Client.PutObjectResult
        public let sha256: String

        public init(put: TideS3Client.PutObjectResult, sha256: String) {
            self.put = put
            self.sha256 = sha256
        }
    }
    ...
    public struct ResumeContext: Sendable {
        ...
        public let store: any UploadCheckpointStore

        public init(path: String, fileMtime: Double, fileSize: Int64, store: any UploadCheckpointStore) {
            self.path = path
