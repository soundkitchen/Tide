...
- **アーキテクチャ**: macOS 26+ 専用、Swift 6（strict concurrency: complete）、SwiftUI、メニューバー常駐 (`LSUIElement = YES`)、App Sandbox 有効（M5 Phase 2〜）。稼働形態は fpOnly（File Provider）のみ = 同期面は FP レプリカ（`~/Library/CloudStorage/Tide` ドメイン）。**v0.3.0 で新規セットアップは security-scoped bookmark を発行しない**（folderSync 世代の bookmark 解決系はデッドコード温存・`security/low.md` L1）。
- **設計の原典**: `docs/README.md`（概観・索引）と `docs/01`〜`07`。**仕様に迷ったら先に docs/ を読むこと。**（v0.3.0 docs 監査で `00` は `docs/README.md` へ統合・`05` は削除・folderSync 世代の同期ロジックは `04a` へ退避 = 削除予約）
- **マイルストーン**: 
...
- `factoryReset` は**アプリから届く範囲で** `make reset` に揃える: App Group コンテナ（DB）/ コンテナ内 Caches / UserDefaults（group + standard）/ Keychain を消し、ログイン項目（`SMAppService`・#116）も外す。sandbox 下では実ホームの旧ロケーション残置分（pre-sandbox の db.sqlite / Caches）に届かない — 完全削除は `make reset`（sandbox 外）のみ。→ `docs/08`
...
| `TideFileProvider/` | **File Provider 拡張**（`NSFileProviderReplicatedExtension`・M5 Phase 3〜）。列挙 / オンデマンド実体化 / 双方向書込（Phase 5）= 現行の同期実体。TideCore に依存・DB 非接触 |
| `TideTests/` | ユニットテスト |
| `tools/soak/` | soak（#40）支援: 整合性突合スクリプト `consistency_check.py`（`make soak-check`。アプリと独立実装・読み取り専用） |
| `docs/` | 設計書（spec）。`README.md` が概観・索引、`01`〜`07` が仕様（`00` は README へ統合・`05` は削除済み・`04a` = folderSync 世代の退避記録 = コード物理撤去と同時に削除）、`08-IMPLEMENTATION-NOTES.md` が実装ノート（旧 §7）、`09-DEFERRED.md` が据え置き/バックログ（旧 §8） |
...
- **[entry 生成はファクトリ経由]**（#118）: S3 書込結果（PUT / MPU complete / CopyObject）から `ManifestFileEntry` を作る経路は必ず `ManifestFileEntry.uploaded(...)` / `.copied(...)`（`TideCore/S3/ManifestFileEntryFactory.swift`）を通す。素の `init` 直呼びは `ManifestReader` の DB キャッシュ再構成のみ。entry 契約変更はファクトリ + `ManifestFileEntryFactoryTests` を直せば全書き手（app / core / FP 拡張）へ届く＝手動反映漏れの無音乖離を構造的に防ぐ。→ `docs/08`
