...
- **中断ダウンロードが「再起動だけでは自動再開しない」（2026-06-05・受け入れテストで発見・未修正）**: `Downloader` の Range 再開機構自体は正しい（実機確認済み）が、**それを呼ぶ pull 経路が中断ファイルを見ない**。`ManifestReader.read()` は**シャードを fetch した時点で `shard_state` に「取得済み」を記録**する（DL 完了前）。DL が途中で中断されると、(1) そのシャードは「未変化」とキャッシュ判定されて次回 pull で S3 から再取得されず、未変化シャードのエントリは**ローカル DB から再構築**されるが、(2) 中断ファイルは DL 未完で**ローカル DB レコードが無い** → reconcile 対象に現れず → `Downloader.download` が呼ばれず Range 再開に到達しない。結果、`transfer_state` 行と部分 tmp は残るが誰も再開せず、**シャードがリモートで変わるか 7 日 stale prune まで取り残される**（クリーンインストール復旧が中断すると一部ファイルが永久欠落しうる＝重要度は高め）。修正案: ①起動時に `transfer_state` の download 行を pull とは独立に再開キックする経路を設ける、②`shard_state` の記録を「シャード内全ファイルの reconcile 完了後」に遅延、③pull で「未完の download 行を持つファイル」は cached シャードでも reconcile に含める、のいずれか。別タスクに据え置き。
