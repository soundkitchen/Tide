...
- **手動 pull は coalescing（2026-06-07・PR #9 レビュー ④）**: pull 進行中の再入のうち**手動（`reason == "manual"`）だけは pending 化し、現 pull 終了後にもう 1 周**する（reason は `manual-coalesced`）。poll/wake/network は次の周期が来るので従来どおりドロップ。`isRemotePulling` は `@Observable` 公開状態（`private(set)`）で、「Pull from S3」ボタンは pull 中スピナー + 「Pulling…」に切り替え（**enabled のまま**＝押下が coalescing の入口。disabled にすると pending 化の入口が無くなる）。
