...
- **中断・再開（サブタスク D・進行中）**: D1（`transfer_state` テーブル + `TransferStateStore`）と D2（アップロードの再起動またぎ再開）は ✅ 実装済み（2026-06-04）。`Uploader.processUpload` が `MultipartUploader.ResumeContext` を渡し、mtime/size 一致なら前回 UploadId と完了パートを引き継いで未送分だけ送る（既送分も読み順に hash 更新して全体 SHA を復元）。**残**: D3 Range ダウンロード再開 / D4 進捗 UI（メニューバーポップオーバー） / D5 起動時オーファン掃除 + セキュリティレビュー。詳細は `docs/07-M3-IMPLEMENTATION-GUIDE.md` サブタスク D。
