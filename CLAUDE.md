...
- **DL prune の clear 分岐がシャードキャッシュを invalidate しない（2026-06-07 受け入れテスト §6-2 で発見・未修正）**: DL 中断後に tmp が消えている（または stale）場合、`pruneOrphanTransfers` は `transfer_state` 行を削除するだけで `invalidateShardCache(forPath:)` を呼ばない → FileRecord 無し + `shard_state` は実 etag のまま → 当該ファイルはシャードがリモートで変化するまで**永久に再 DL されない**。修正形は明確: clear 分岐にも再 arm 分岐（resumable 分岐）と対称に invalidate を足す + 回帰テスト。次タスクで対応予定。
