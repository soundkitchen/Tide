...
- **中断ダウンロードの自動再開（2026-06-05・受け入れテストで発見 → ✅ 修正済み）**: 旧挙動では中断 DL が**再起動だけでは自動再開しなかった**。`Downloader` の Range 再開機構自体は正しいが、それを呼ぶ pull 経路が中断ファイルを見なかった: `ManifestReader.read()` は**シャードを fetch した時点で `shard_state` に「取得済み」を記録**する（DL 完了前）ため、DL が中断されると (1) そのシャードは次回 pull で「未変化」とキャッシュ判定され S3 から再取得されず未変化シャードは**ローカル DB から再構築**されるが、(2) 中断ファイルは DL 未完で**ローカル DB レコードが無い** → reconcile されず → `Downloader.download`（Range 再開）に到達せず、`transfer_state` 行と部分 tmp が取り残された（クリーンインストール復旧が中断すると一部ファイルが永久欠落しうる重大バグ）。**修正（方針④）**: `SyncEngine.pruneOrphanTransfers` で、prune されない（= tmp あり・新しい＝再開可能な）download 行について、その path のシャードの `shard_state` 行を削除（invalidate）する。起動時 prune は pull の前に走るので、直後の起動 pull がそのシャードを S3 から再取得 → 中断ファイルを reconcile → 既存 tmp で Range 再開、という既存機構をそのまま再利用する（新たな再開コード経路を増やさない）。同シャードの他ファイルは reconcile で localMatchesRemote → no-op で安全。実機で「shard_state 手動クリア無し・再起動のみ」で Range 再開・SHA 一致を確認。再 arm は行を削除せず etag を空 sentinel に更新する（removed-shard 検出を温存。PR #9 レビュー ③）。
  - **残ギャップ（PR #9 レビュー ②・follow-up・未修正）**: 再 arm は**起動時 `pruneOrphanTransfers` でのみ**走る。**セッション中**に DL がネットワーク断等で失敗した場合、`ManifestReader` が fetch 時点で `shard_state` を記録済み＋失敗ファイルは FileRecord 無しのため、その後の poll/wake/network-up pull では同シャードがキャッシュ済み扱いで再開されず、**シャードがリモートで変化するか再起動するまで取り残される**（再起動で自己回復する分だけ軽い）。同じ機構の延長で塞げる: `Downloader.download` のネットワーク失敗 catch（`recordDownloadProgress` 箇所）または `reconcileRemoteEntry` の catch で当該 path のシャードの `shard_state` を sentinel 化すれば、次の poll/network-up pull が再取得→Range 再開する。
- **手動「S3 から取得」の pull 中サイレントドロップ（PR #9 レビュー ④・follow-up・未修正）**: pull の単一ゲート化により、pull 進行中に「S3 から取得」を押しても `remotePullInFlight` で無反応・無表示でドロップされる（poll が 3 分ごとに来るので実害は限定的）。pending フラグを立てて現 pull 終了後にもう 1 周する coalescing か、UI フィードバックを検討。
