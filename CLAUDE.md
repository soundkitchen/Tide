...
- **[ネスト .syncignore]** ディレクトリごとの `.syncignore` を git 風に階層適用（[#27] / C1）。`shouldSkip` の `matcher` は `LayeredSyncIgnore`（dir→`SyncIgnoreMatcher` 辞書）で、祖先 dir を**浅い→深い順**に合成し深い層が浅い層を上書きする（`SyncIgnoreMatcher.evaluate` は三状態）。**辞書の再構築（#64・単一走査化後）**: 評価経路は I/O ゼロのまま、起動時・ローカル `.syncignore` 変更時は**フルスキャンの走査副産物**（`singlePassScan` が再帰下降で層辞書も構築し scan 完了時に世代ガード付き publish・先行 discovery 走査なし）+ ローカル変更は**変更 1 枚のインプレース patch**（`patchIgnoreLayer`・走査ゼロで旧 matcher 窓を閉じる）。リモート由来は pull が `.syncignore` を触ったときだけ `reloadIgnoreMatcher()`（`loadLayeredIgnore` はこの経路専用）。定常 pull では再走査しない。走査は symlink 非追従・機密網 dir subtree 非降下・`maxFiles` で有界（上限超過でも scan は全域走査を続け層の追加だけ打ち切る）。→ `docs/07` サブタスク B / `docs/08`「フルスキャンの単一走査化」
