...
**更新 (2026-07-21・#64 / PR #74):** フルスキャンの走査本体は enumerator から**ディレクトリ再帰下降**（`SyncEngine.walkSyncTree` + `classifyAndEnqueue`・`SyncEngine+FullScan.swift`）へ書き換えた。symlink（dir リンク含む）は**走査スタックへ push しない**＝構造的に降りない（`skipDescendants()` 自体を使わないため Issue #54 型の誤用は起こり得ない）。機密網ディレクトリも subtree ごと降りない（旧実装の per-file フィルタと検出同値・コンポーネント単位判定のため）。`loadLayeredIgnore`（pull 末尾専用に残置）は enumerator ベースのまま＝上記 #54 の規約が引き続き適用される。event 経路の `.syncignore` インプレース patch（`patchIgnoreLayer`）も走査と同一ゲート（共通実装 `readSyncignoreLayer` = `resolveSafely` + **lstat 相当の regular file 確認〈symlink / FIFO 拒否・FIFO は open すると永久ブロックで DoS になるため open 前に弾く（PR #74 レビュー高 1）〉** + 256KB 上限）を通す。終端不変条件は `FullScanSymlinkTests` + `FullScanSinglePassTests` + `SyncEngineIgnorePatchTests` で固定。

**該当箇所（対応コード）:** フルスキャン = `Tide/Core/SyncEngine+FullScan.swift` の `walkSyncTree`（symlink を push しない再帰下降・#64）。`.syncignore` discovery = `Tide/Core/SyncEngine.swift` の `loadLayeredIgnore`（enumerator 構築 + `.isSymbolicLinkKey` 取得 → symlink は **`continue` のみ**。`skipDescendants()` は「現在 item がディレクトリ」の機密網 dir スキップ＝`loadLayeredIgnore` の `HardcodedIgnoreRules` 分岐でのみ使用 — Issue #54）。Downloader の書き込み先 symlink 拒否（`Downloader.download` 入口）、`applyRemoteDeletion` の symlink 除外。以下は対策前の、symlink を辿っていたコード（参考）:
