...
enum BucketPolicyBuilder {
    /// Tide が管理する statement の固定 Sid。これを持つ statement だけを差し替え対象とみなす。
    static let tideDenySid = "TideDenyInsecureTransport"

    enum BucketPolicyError: Error, CustomStringConvertible {
    ...
    static func denyStatement(bucket: String) -> [String: Any] {
        [
            "Sid": tideDenySid,
            "Effect": "Deny",
            "Principal": "*",
    ...
    static func mergeTLSDenyStatement(into existing: String?, bucket: String) throws -> String {
        var root: [String: Any] = [:]
    ...
    static func isTLSDenyEnforced(in policy: String?, bucket: String) -> Bool {
